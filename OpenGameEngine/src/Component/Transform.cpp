...
namespace openge {
	...
	void Transform::lookAt(const Vector3& target, float speed)
	{
		...
		/*
		* QuaternionLookAt:
		A função QuaternionLookAt é parte da biblioteca GLM (OpenGL Mathematics) e é usada para calcular uma rotação de quaternião com base em uma direção desejada. Ela recebe dois parâmetros: direction (direção) e up (vetor "para cima").
